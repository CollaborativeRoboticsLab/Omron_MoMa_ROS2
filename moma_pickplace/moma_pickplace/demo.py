...
from gripper_core import Pickplace_Driver, Transform, TM_Exception
from gripper_msgs.srv import GetPostion
from gripper_msgs.msg import MovePosition

from amr_msgs.goto_goal import AmrActionClient
...
class Coordinates:
    def __init__(self, mode):
        with open(mode + '_config.txt') as json_file:
...
class TMHandler:
    def __init__(self, node, pickplace_driver):
        ...
        self.cli = node.create_client(GetPostion, 'ask_modbus')
        self.flagpublisher = self.node.create_publisher(MovePosition, 'objectflag', 10)
...
def main():
    ...
    cli = node.create_client(GetPostion, 'ask_modbus')
    while not cli.wait_for_service(timeout_sec=1.0):
        print("Service not available...")
    req = GetPostion.Request()
    req.req = 'init_io'
    ...
    try:     
        ...
        flagpublisher = node.create_publisher(MovePosition, 'objectflag', 10)
        msg = MovePosition()
        msg.parent = "world"
